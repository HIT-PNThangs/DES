...
class Des:
    ...
    def thuchienmahoa(self, pt, key) -> string:
        ...
        cipher_text = self.bin2hex(self.encrypt(pt, rkb, rk))

        return cipher_text

    def thuchiengiaima(self, cipher, key) -> string:
        key = self.hex2bin(key)
        key = self.permute(key, self.keyp, 56)
        left = key[0:28]
        right = key[28:56]
        ...
        rkb_rev = rkb[::-1]
        rk_rev = rk[::-1]
        text = self.bin2hex(self.encrypt(cipher, rkb_rev, rk_rev))
        return text